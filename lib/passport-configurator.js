...
PassportConfigurator.prototype.configureProvider = function(name, options) {
  ...
  var defaultCallback = function(req, res, next) {
    options.cookieAccessToken = options.cookieAccessToken ?
      options.cookieAccessToken : 'access_token';

    options.cookieUserId = options.cookieUserId ?
      options.cookieUserId : 'userId';
          ...
          req.logIn(user, function(err) {
            ...
            if (info && info.accessToken) {
              if (!!options.json) {
                ...
                res.cookie(options.cookieAccessToken, info.accessToken.id,
                  {
                ...
                res.cookie(options.cookieUserId, user.id.toString(), {
                  signed: req.signedCookies ? true : false,
                  maxAge: 1000 * info.accessToken.ttl,
                  domain: (options.domain) ? options.domain : null,
                });
